...
contains
  ...
  subroutine c_macom_finalize_mpi() &
    ...
    ! Use mpi_final_operations for finalization
  ...
  !-----------------------------------------------------------------------------
  ! 4D-Var DA Main (C++ interface) - Empty implementation for globe mode
  !-----------------------------------------------------------------------------
  subroutine c_macom_4var_da_main() &
    bind(C, name="c_macom_4var_da_main")
    ! Empty implementation for globe mode
    ! This function is required for linking but not used in globe mode
    if (mpi_rank .eq. 0) then
      call macom_log_info("FortranWrapper", &
                          "c_macom_4var_da_main called (empty implementation for globe mode)")
    end if
